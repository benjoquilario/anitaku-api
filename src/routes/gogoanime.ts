...
import { extract as extractGogoCDN } from "../extractors/gogocdn"
import { extract as extractStreamSB } from "../extractors/streamsb"
...
export const search = async function (query: string, page: number = 1) {
    ...
    switch (server) {
      case StreamingServers.GogoCDN:
        return {
          ...
          sources: await extractGogoCDN(serverUrl),
      ...
      case StreamingServers.StreamSB:
        return {
          ...
          sources: await extractStreamSB(serverUrl),
          download: downloadUrl
            ? downloadUrl
            : `https://${serverUrl.host}/download${serverUrl.search}`,
        }
